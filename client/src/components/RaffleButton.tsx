...
const RaffleButton: React.FC = () => {
  ...
  const handleRaffleEntry = async () => {
    ...
    try {
      ...
      if (result.success && result.tickets !== undefined) {
        ...
        setMessage({
          text: `You have ${result.tickets} ticket${
            result.tickets !== 1 ? "s" : ""
          }`,
          type: "success",
        });
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-md p-4 mb-6">
      <h3 className="font-bold text-lg mb-3 text-navy-900">
        Monthly Support Raffle
      </h3>

      <p className="text-gray-600 mb-4 text-sm">
        Join our monthly raffle for a chance to win a free counseling session or
        wellness package.
      </p>
      ...
      {status && (
        <div className="flex items-center mb-3 text-sm">
          ...
          <span>
            You currently have {status.tickets} ticket
            {status.tickets !== 1 ? "s" : ""}
          </span>
        </div>
      )}
